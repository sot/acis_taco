...
def calc_earth_vis(p_chandra_eci,
    ...
    for rad_x in RAD_XS:
        for rad_y in RAD_YS:
            for i in numpy.arange(n_rays)
                ray_x = rays_x_all[i]
                ray_y = rays_to_earth[i, 1]
                ...
                for refl in range(max_reflect + 1):
                    ...
                    if y_z_ok:
                        d_vis = ray_z * REFLECT_ATTEN**refl / N_RAD_POINTS
                        illum[refl] += earth_solid_angle * numpy.sum(d_vis) / n_rays
                        break  # ray escaped, don't consider further reflections
...
RAD_XS, RAD_YS = make_radiator(10, 20)
